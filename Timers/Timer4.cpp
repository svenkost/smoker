...
void TimerFour::initialize(long microseconds)
{
  ...
  TCCR4B = 0;        // set mode as phase and frequency correct pwm, stop the timer
...
{
  TIMSK4 &= ~_BV(OCIE4A);                                   // clears the timer overflow interrupt enable bit
}
